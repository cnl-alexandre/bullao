    ...
    <div class="container" style="max-width: 380px!important; margin: 22vh auto auto auto;">
        ...
        <div style="background-color: #fff;border-radius: .25rem;padding: .2rem .5rem;text-align: center">
            <p class="font-size-14">
                {{ $carte->cadeau_offre }}
                <br>Montant total : {{ $carte->cadeau_montant }}€
            </p>
        </div>

        <form action="{{ url('/cartecadeau/paiement') }}" method="post" id="payment-form" style="margin-top: 3vh;">
            {{ csrf_field() }}
            <div class="form-row" >
                <div id="errors"></div>
            ...
            <input type="hidden" name="cadeau_id" value="{{ $carte->cadeau_id }}">
