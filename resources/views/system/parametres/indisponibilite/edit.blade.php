...
@section('pageTitle', 'Nouvelle Indisponibilite  Bullao | '.env('APP_NAME'))
...
<form action="{{ $action }}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-md-7 text-left">
            @if(isset($indispo))
                <h1 class="h3 mb-2 text-gray-800">Modifier la date indisponible</h1>
                <p class="mb-4">Dernière mise à jour le : {{ $indispo->dateUpdated->format('d-m-Y') }}</p>
            @else
                <h1 class="h3 mb-2 text-gray-800">Ajouter une date indisponible</h1>
                <p class="mb-4">Renseigner les informations suivantes pour ajouter une date indisponible à la liste</p>
            @endif
