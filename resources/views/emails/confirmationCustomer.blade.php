<!DOCTYPE html>
<html>
			<head>
					<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
					<title>Nouvelle réservation</title>
					<meta name="viewport" content="width=device-width, initial-scale=1.0">
			</head>
			<body style="font-family:Oxygen, 'Helvetica Neue', Helvetica, Arial, sans-serif;background-color: #f4f4f4;width: 100%;margin: 0;padding: 0;">

					<table width="100%" bgcolor="#f4f4f4" cellpadding="0" cellspacing="0" border="0">
							<tbody>
									<tr>
											<td style="padding: 0 0 40px 0;">
													<!-- begin main block -->

													<table cellpadding="0" cellspacing="0" width="400" border="0" align="center">
															<tbody>
																	<tr>
																			<td>

																					<table width="100%" bgcolor="#fff" border="0">
																							<tbody>
																									<tr>
																											<td width="100%" height="80" bgcolor="#194F9A" style="text-align: center;">
																													<a href="{{ url('/') }}" rel="noopener noreferrer" target="_blank" style="text-decoration: none;">
																															<img src="http://www.bullao.fr/assets/logo-blanc.png" alt="Logo Bullao" width="180">
																													</a>
																											</td>
																									</tr>
																									<tr>
																											<td width="100%" bgcolor="#fff" style="text-align: center;">
																													<h1 style="color: #194F9A;font-size: 1.4rem;font-weight: lighter;margin-top: 25px;">Validation de votre commande</h1>
																											</td>
																									</tr>
																									<tr>
																											<td width="100%" bgcolor="#fff">
																													<p style="line-height: 160%;padding: 0 1.5rem;margin-bottom: 25px;">
																															Bonjour,
																															<br><br>Nous avons le plaisir de vous confirmer la validation de votre commande sur le site bullao.fr.
																															<br><br>Vos informations de livraisons ont bien été enregistrées dans notre base de données sécurisée.
																															<br><br>Si des informations dans votre dossier sont menées à être modifiées, vous êtes priées de nous contacter pour corriger l’information.
																															<br>
																													</p>
																											</td>
																									</tr>
																									<tr bgcolor="#fff">

																										<table bgcolor="#f4f4f4" style="padding: 1rem;margin: auto;">
																												<tbody>
																														<tr >
																																<td>Récapitulatif de l'achat</td>
																														</tr>
																														<hr style="width: 115%;opacity: 0.4;">
																														<tr>
																																<td width="50%">Formule 1 soirée</td>
																																<td width="50%" style="text-align: end;">90€</td>
																														</tr>
																														<tr>
																																<td width="70%">Spa Baltik 4 places</td>
																														</tr>
																														<tr>
																																<td width="88%">Du 01/10/2020 au 03/10/2020</td>
																														</tr>
																														<tr>
																																<td width="50%">Pack Fun</td>
																																<td width="50%" style="text-align: end;">+ 20€</td>
																														</tr>
																														<tr>
																																<td width="50%">Enceinte portable</td>
																																<td width="50%" style="text-align: end;">+ 9€</td>
																														</tr>
																														<hr style="width: 115%;opacity: 0.4;">
																														<tr style="color: #194F9A;">
																																<td width="50%">Montant total</td>
																																<td width="50%" style="text-align: end;">119€</td>
																														</tr>
																												</tbody>
																										</table>

																									</tr>
																									<tr>
																											<td width="100%" bgcolor="#fff">
																													<p style="line-height: 160%;padding: 0 1.5rem;margin: 25px 0;">
																															Merci pour votre achat et de la confiance que vous m’accordez.
																															<br><br>Grégoire de Bullao.
																													</p>
																											</td>
																									</tr>
																							</tbody>
																					</table>

																					<p style="color: #7D7D7D;text-align: center;margin-top: 35px;">©Copyright 2020 - Bullao</p>

																			</td>
																	</tr>
															</tbody>
													</table>

													<!-- end main block -->
											</td>
									</tr>
							</tbody>
					</table>

			</body>
</html>
